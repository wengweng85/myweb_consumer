...
$(function() {
	...
	window.onscroll=function(){
		rc.getScrollTop()>0?$('#goTopBtn').show():$('#goTopBtn').hide()
	};
	...
	if($('#goTopBtn').length>0){
		...
		$('#goTopBtn').mouseleave(function(){
			$('#goTopBtn').removeClass('hover');
		});
		$('#goTopBtn').mouseenter(function(){
...
var rc = {
   ...
   setTab:function(name,cursel,n){
	...
	gotoP:function(pageNumber){
		var n=$('#gtop').val();if(n){if(isNaN(n)){alert('请输入正确的页码！');}
		else{if(n>pageNumber||n<=0){alert('此页码不存在！')}else{rc.page(n-1);}}}else{alert('请输入正确的页码！');}
	},
	...
	upload:function(button,filetype,type,business_id,callback){
		...
		if(filetype=='01'){
			...
			fileTypeExts ='jpg|jpeg|JPG|JPEG|gif|GIF|png|GNG|doc|DOC|docx|DOCX|pdf|PDF|xls|XLS|xlsx|XLSX';
		}else{
			button.val('文件类型不正确!');
			alert('文件类型不正确!');
			return false;
		...
		var url=contextPath+'/upload?business_id='+business_id+'&type='+type;
...
(function($){
	$.fn.paging = function(model,target,settings) {
		...
		if(_this.settings.pageSize){
			param+='pageSize='+_this.settings.pageSize;
		}else{
			param+='pageSize=10';
		}
		if(_this.settings.curPage){
			param+='&curPage='+_this.settings.curPage;
		}else{
			param+='&curPage=0';
		}
		...
		var pages = ['6','10','15','20','30','40'];
		rc.ajax(_this.settings.url,param,function(response){
			var jsondata=response.obj;
			var modeldata=Handlebars.compile(model.html());
			var views = modeldata(jsondata);
			target.html(views);
			_this.settings.curPage=response.obj.curPage;
			_this.settings.pageNumber=response.obj.pageNumber;
			_this.settings.totalCount=response.obj.totalCount;
			var pagehtml="";
			if(parseInt(_this.settings.totalCount,10)>0){
				pagehtml+="显示数量：<select id='pageSize' onchange=\"$.fn.paging.choosePageSize('"+_this.selector+"','"+model.selector+"','"+target.selector+"','"+_this.settings.url+"','"+0+"','"+_this.settings.pageSize+"','"+_this.settings.formid+"',"+_this.settings.ismask+")\">";
				...
				pagehtml+="</select>&nbsp;总记录数:<span style='color:red;font-weight:bold;'>"+_this.settings.totalCount+"</span>&nbsp;";
			}else{
				pagehtml+="<span style='color:red;font-weight:bold;'>没有找到数据!<input type='hidden' id='pageSize' value='"+_this.settings.pageSize+"'/></span>"
			}

			if(parseInt(_this.settings.pageNumber,10)>1){
				if(_this.settings.curPage==0){
					pagehtml+="<a class='page' style='background-color: #ccc;'>首&nbsp;页</a>&nbsp;";
					pagehtml+="<a class='page' style='background-color: #ccc;'>上一页</a>&nbsp;";
				}else{
					pagehtml+="<a class='page' href='javascript:;' onclick=\"$.fn.paging.next('"+_this.selector+"','"+model.selector+"','"+target.selector+"','"+_this.settings.url+"','"+0+"','"+_this.settings.pageSize+"','"+_this.settings.formid+"',"+_this.settings.ismask+")\">首&nbsp;页</a>&nbsp;"
					pagehtml+="<a class='page' href='javascript:;' onclick=\"$.fn.paging.next('"+_this.selector+"','"+model.selector+"','"+target.selector+"','"+_this.settings.url+"','"+(parseInt(_this.settings.curPage)-1)+"','"+_this.settings.pageSize+"','"+_this.settings.formid+"',"+_this.settings.ismask+")\">上一页</a>&nbsp;"
				}
				var begin=_this.settings.curPage-2;
				var end=_this.settings.curPage+3;
				if(_this.settings.curPage<2){
					begin=0;
					end=_this.settings.pageNumber>5?5:_this.settings.pageNumber;
				}
				if(_this.settings.curPage>_this.settings.pageNumber-3){
					begin=_this.settings.pageNumber-5<0?0:_this.settings.pageNumber-5;
					end=_this.settings.pageNumber;
				}
				for (var i=begin;i<end;i++){
					if(_this.settings.curPage==i){
						pagehtml+="<span class='current'>"+(i+1)+"</span>&nbsp;"
					}else{
						pagehtml+="<a href='javascript:;' onclick=\"$.fn.paging.next('"+_this.selector+"','"+model.selector+"','"+target.selector+"','"+_this.settings.url+"','"+i+"','"+_this.settings.pageSize+"','"+_this.settings.formid+"',"+_this.settings.ismask+")\">"+(i+1)+"</a>&nbsp;"
					}
				}
				if(_this.settings.curPage==_this.settings.pageNumber-1){
					pagehtml+="<a class='page' style='background-color: #ccc;'>下一页</a>&nbsp;";
					pagehtml+="<a class='page' style='background-color: #ccc;'>尾&nbsp;页</a>&nbsp;";
				}else{
					pagehtml+="<a class='page' href='javascript:;' onclick=\"$.fn.paging.next('"+_this.selector+"','"+model.selector+"','"+target.selector+"','"+_this.settings.url+"','"+(parseInt(_this.settings.curPage)+1)+"','"+_this.settings.pageSize+"','"+_this.settings.formid+"',"+_this.settings.ismask+")\">下一页</a>&nbsp;";
					pagehtml+="<a class='page' href='javascript:;' onclick=\"$.fn.paging.next('"+_this.selector+"','"+model.selector+"','"+target.selector+"','"+_this.settings.url+"','"+(parseInt(_this.settings.pageNumber)-1)+"','"+_this.settings.pageSize+"','"+_this.settings.formid+"',"+_this.settings.ismask+")\">尾&nbsp;页</a>";
				}
				//pagehtml+="|"
				//pagehtml+="&nbsp;&nbsp;当前页次<span style='color:red;font-weight:bold;'>"+(_this.settings.curPage+1)+"</span>/"+_this.settings.pageNumber+"页";
			}
			_this.html(pagehtml);
	 ...
	 */
	$.fn.paging.next = function(pag,model,target,url,curpage,pagesize,formid,ismask) {
		$(pag).paging($(model),$(target),{url:url,curPage:curpage,pageSize:pagesize,formid:formid,ismask:ismask});
	}
	$.fn.paging.choosePageSize = function(pag,model,target,url,curpage,pagesize,formid,ismask){
		$.fn.paging.next(pag,model,target,url,curpage,$('#pageSize').val(),formid,ismask);
	}
	
	/**
	 * default param
	 * @type {{url: string, formid: string, curPage: number, pageSize: number}}
	 */
	$.fn.paging.defaults={
		url:'',
		formid:'',
...
function ACA111Select(type){
	layer.open({
	    ...
	    scrollbar: false,
	    area: ['700px', '550px'],
	    content: contextPath+'/common/toOpenAca111?type='+type
	});
}

//工种选择
function AbiltyACA111Select(type){
	layer.open({
	    type: 2,
	    title: '选择专项能力项目',
	    fix: false,
	    ...
	    scrollbar: false,
	    area: ['700px', '550px'],
	    content: contextPath+'/common/toOpenAblityAca111?type='+type
	});
}

//工种选择
function newACA111Select(type){
	layer.open({
	    type: 2,
	    title: '选择工种',
	    fix: false,
	    ...
	    scrollbar: false,
	    area: ['700px', '550px'],
	    content: contextPath+'/common/toOpenNewAca111?type='+type
	});
}

//重置工种等级
function changeAca11a(ACA111,type){
	$.ajax({
			type: 'POST',
			url: contextPath+'/common/getAca11a',
			dataType: "json",
			data:"ACA111="+ACA111+"&type="+type,
			success: function(response){
				$("#ACA11A option").remove(); 
				document.getElementById("ACA11A").options.add(new Option("请选择等级",""));
				$.each(response.obj, function(i, m) {
					document.getElementById("ACA11A").options.add(new Option(m.NAME,m.ID));
				});
			}
		});
}
...
function TeacherSelect(){
	layer.open({
	    ...
	    scrollbar: false,
	    area: ['700px', '550px'],
	    content: contextPath+'/common/toOpenAca111'
	});
}
...
function SelectMajor(){
	layer.open({
	    ...
	    scrollbar: false,
	    area: ['350px', '550px'],
	    content: contextPath+'/common/major/init'
	});
}
...
function lookPost(CHE200){
	layer.open({
	    ...
	    scrollbar: false,
	    area: ['700px', '400px'],
	    content: contextPath+'/common/postLook/'+CHE200
	});
}
